...
[ "___indirect_function_table", "__emscripten_proxy_main", "_main", "onRuntimeInitialized" ].forEach(prop => {
...
if (!ENVIRONMENT_IS_PTHREAD) {
  if (Module["wasmMemory"]) {
    ...
    var INITIAL_MEMORY = Module["INITIAL_MEMORY"] || 131072;
...
function findWasmBinary() {
  if (Module["locateFile"]) {
    var f = "find_min_global_thread.wasm";
    if (!isDataURI(f)) {
      return locateFile(f);
    }
    return f;
  ...
  return new URL("find_min_global_thread.wasm", import.meta.url).href;
}
...
var PThread = {
  ...
  allocateUnusedWorker() {
    ...
    worker = new Worker(new URL("find_min_global_thread.js", import.meta.url), workerOptions);
    PThread.unusedWorkers.push(worker);
  },
  getNewWorker() {
...
export default createFindMinGlobalModule;
var isPthread = globalThis.self?.name?.startsWith('em-pthread');
var isNode = typeof globalThis.process?.versions?.node == 'string';
if (isNode) isPthread = (await import('worker_threads')).workerData === 'em-pthread';

// When running as a pthread, construct a new instance on startup
isPthread && createFindMinGlobalModule();
